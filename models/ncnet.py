...
from model_utils import Softmax1D,Conv4d
...
class ScanRegNet(nn.Module):
    def __init__(self, 
        ...
        if not self.train_fe:
            encoder_params = list(self.dxa_encoder.parameters()) + list(self.mri_encoder.parameters()) 
            for p in encoder_params:
                p.requires_grad = False

    def forward(dxa_vol, mri_vol):
        dxa_spatial_embeds = self.dxa_encoder.module.transformed_spatial_features
...
if __name__ == '__main__':
    ...
    USE_CUDA = False
    dxa_model, mri_model, val_stats, epochs =  load_models(LOAD_FROM_PATH, 'max',USE_CUDA)
    ds = MidCoronalDataset()
    sample=ds[0]


    sr_net = ScanRegNet(dxa_model, mri_model)
