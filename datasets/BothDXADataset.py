...
class BothDXADataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            dxa_bone_path = os.path.join(self.dxa_root,'bone', scan_row['dxa_filename'] + '.mat')
            dxa_tissue_path = os.path.join(self.dxa_root,'tissue', scan_row['dxa_filename'] + '.mat')
            dxa_bone_img  = torch.Tensor(loadmat(dxa_bone_path)['scan']).float()
            dxa_tissue_img  = torch.Tensor(loadmat(dxa_tissue_path)['scan']).float()
            mri_img  = torch.load(mri_path)[None]
        except Exception as e: 
            print(e)
            exit()
            return BothDXADataset.__getitem__(self, np.random.randint(self.scans_df.__len__()-1))

        dxa_img = torch.stack([dxa_bone_img, dxa_tissue_img], dim=0)
        ...
        if dxa_img.shape != output_dxa_shape:
        ...
        if mri_img.shape != output_mri_shape:
...
class BothDXAHardNegativesDataset(BothDXADataset):
    def __init__(self, hard_negative_list: list, set_type='train',
                 ...
                 scan_lists_path='/users/rhydian/self-supervised-project/scan_lists',
                 augment=False, blanking_augment=False):
        super(BothDXAHardNegativesDataset, self).__init__(set_type, all_root, mri_root, dxa_root, 
...
if __name__ == '__main__':
    ...
    unaugmented_ds = BothDXADataset(augment=False, blanking_augment = False)
