...
def get_matching_keypoints_correspondance(img1, img2, enc_model1, enc_model2, use_cuda, upsample_factor, 
                           use_cyclic, num_points,use_ransac, ransac_tolerance, grid_res, allow_scaling):
    # encode scans
    enc_model1.eval()
    enc_model2.eval()
    img1 = img1[None]
    ...
    with torch.no_grad():
        ses1 = F.normalize(enc_model1(img1),dim=1)
        ses2 = F.normalize(enc_model2(img2),dim=1)
    ...
    corr4d = get_4d_corr(ses1, ses2)
    # make mesh in source image as candidate points to be salient
    X,Y =np.meshgrid(np.linspace(10,img1.shape[-2]-10,int(80*grid_res)),np.linspace(10,img1.shape[-1]-10,int(40*grid_res)))
    points = np.stack((X.reshape(-1),Y.reshape(-1))).transpose()
    points = points[np.random.permutation(np.arange(points.shape[0]))]
    # find salient pairs
    img1_sal_pts, img2_sal_pts, correlations = get_most_discriminative_points(points,corr4d[0],img1[0], img2[0],num_points,use_cyclic)
    if use_ransac:
        img1_inliers, img2_inliers, M = fit_RANSAC(img1_sal_pts, img2_sal_pts, ransac_tolerance)
        M, scaling, angle, T = find_best_rigid(img1_inliers, img2_inliers, allow_scaling)
        matching_error = find_matching_error(img1_inliers, img2_inliers, M)

    else:
        img1_inliers, img2_inliers = img1_sal_pts, img2_sal_pts
        #M, scaling, angle, T = find_best_rigid(img1_inliers, img2_inliers, allow_scaling)
        #matching_error = find_matching_error(img1_inliers, img2_inliers, M)
        M, scaling,angle,T= (0,0,0,0)
        matching_error=None

    return img1_inliers, img2_inliers, M, scaling, angle, T, matching_error
...
def LowesTest(img1,img2,model1,model2,threshold,use_ransac,ransac_tolerance,use_cuda):
    if use_cuda:
        img1 = img1.cuda()
        img2 = img2.cuda()
    else:
    ...
    return ransac_good, np.linalg.inv(M)

def RefinementNetwork(dxa_img, mri_img, dxa_model, mri_model,threshold, ransac_tolerance, use_cuda, refinement_model,src_pts):
    ransac_good, M = LowesTest(dxa_img, mri_img,dxa_model,mri_model,threshold,True,ransac_tolerance,use_cuda)
