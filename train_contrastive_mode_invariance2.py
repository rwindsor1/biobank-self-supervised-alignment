...
def load_models(LOAD_FROM_PATH, POOLING_STRATEGY, use_cuda=True):
    dxa_model = SpatialVGGM2(pooling=POOLING_STRATEGY)
    mri_model = SpatialVGGM2(pooling=POOLING_STRATEGY)
...
@ex.capture
def get_embeddings(dxa_model, mri_model, dl, NORMALIZE, use_cached=True):
    dxa_model.eval()
    mri_model.eval()
    dxa_embeddings = []
    mri_embeddings = []
...
@ex.capture
def val_epoch(dxa_model, mri_model,dl, BATCH_SIZE, MARGIN, USE_INSTANCE_LOSS, NORMALIZE, _run):
    val_pbar = tqdm(dl)
    dxa_embeds = []
    mri_embeds = []
    val_losses = []
...
@ex.capture
def train_epoch(train_hard_negatives, dxa_model, mri_model, optim, TRAIN_BATCH_SIZE, TRAIN_NUM_WORKERS, TRAINING_ITERATIONS, DATASET_ROOT, MARGIN, NORMALIZE):
    ...
    for idx, sample in enumerate(pbar):            
        ...
        if NORMALIZE:
            ...
            dxa_embeds1 = dxa_model(dxa_vols1.cuda()).squeeze(-1).squeeze(-1)
            mri_embeds1 = mri_model(mri_vols1.cuda()).squeeze(-1).squeeze(-1)
            dxa_embeds2 = dxa_model(dxa_vols2.cuda()).squeeze(-1).squeeze(-1)
            mri_embeds2 = mri_model(mri_vols2.cuda()).squeeze(-1).squeeze(-1)
